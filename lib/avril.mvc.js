...
function Mvc() {
    ...
    function registerArea(app) {
        ...
        fs.readdirSync(areaDir).forEach(function (area) {
            if(area.indexOf('.') < 0){
    ...
    function registerController(app, dir, area) {
        ...
        fs.readdirSync(dir).forEach(function (controller) {
            if ( fs.statSync(path.join(dir, controller) ).isFile() )  {
    ...
    function parseAction(action, controller, area, fn, ctrlCfgCache, actions) {
        return {
            func: function (req, res, next) {
                ...
                , helper = new Helper(_app, req, res, routes, avril.extend(true,{}, ctrlCfgCache,getAttribute(action, ctrlCfgCache)), mvc);
                ...
                res.view = function () {
                    ...
                    options.authorize = helper.authorize;
                    ...
                    var str = '', statusCode = 200;

                    try {
                        str = helper.viewHelper.render(getPureActionForView(viewName), options);
                    } catch (E) {
                        statusCode = 500;
                ...
                helper.authorize.auth(function (userInfo) {
                    var isLogin = userInfo && userInfo.id;
                    helper.authorize.user = userInfo;
                    if (!isLogin && getAttribute(action, ctrlCfgCache).authorize) {
    ...
    function getAttribute(str, cfg) {
        ...
        if (typeof cfg.authorize !== undefined) {
            if (cfg.authorize === '*' || cfg.authorize === true) {
    ...
    function preCombineJsAndCss(appConfig, scriptResources, styleResources) {
        ...
        scriptResources && Object.keys(scriptResources).forEach(function (key) {
            resourceScript(key, scriptResources[key].minify || appConfig.minifyJs, scriptResources[key].version || appConfig.version, scriptResources[key].items, scriptResources[key].attrs, appConfig.minifyScriptEx, scriptResources[key].copyTo);
        });

        styleResources && Object.keys(styleResources).forEach(function (key) {
            resourceStyle(key, styleResources[key].minify || appConfig.minifyCss, styleResources[key].version || appConfig.version, styleResources[key].items, styleResources[key].attrs, appConfig.minifyCssEx, styleResources[key].copyTo);
        })
    ...
    this.init = function (avril, appConfig) {
        ...
        this.viewEngine = require('../third-party/jshtml/index');

        avril.extend(mvcConfig, appConfig);
        ...
        preCombineJsAndCss(appConfig.appConfig, appConfig.scriptResources, appConfig.styleResources);
...
(function () {
    ...
    ViewHelper.prototype.viewEngine = function (engine) {
        ...
        return this._viewEngine || require('jshtml');
    }
    ...
    ViewHelper.lookup = function (mvc, action, controller, area, reLookup) {
        ...
        if (!viewPathCache[key]) {
            throw new Error('View <b>"' + action + '"</b> not found !\r\n Search these paths:\r\n' + paths.join('\r\n'));
        }
...
(function () {
    ...
    var resourceScriptList
        = HtmlHelper.resourceScriptList
        = HtmlHelper.prototype.resourceScriptList = function (resourceName, combine, version, scriptList, minifyEx, copyTo) {
        ...
        minifyEx = minifyEx || resObj.minifyEx || appConfig.minifyJsEx || '.js';
        ...
        if (scriptList && (scriptList instanceof Array)) {
            if (combine) {
                ...
                var fileName = resourceName + '-' + version + minifyEx;
                    ...
                    fs.exists(releasePath, function (exists) {
                        if (!exists) {
                            minify.minifyJs(_list, function (err, script) {
                                ...
                                if (copyTo) {
                                    copyTo.forEach(function (toPath) {
                                        var releasePath = path.join(avril._rootDir, 'public', toPath, fileName);
                                        fs.mkdirs(path.dirname(releasePath), function () {
                                            fs.writeFile(releasePath, script, 'utf8');
                                        });
                                    });
    ...
    HtmlHelper.prototype.resourceScript = function (resourceName, combine, version, scriptList, attrs, minifyEx, copyTo) {
        var appConfig = Mvc.appConfig.appConfig || {}
        , scriptResources = Mvc.appConfig.scriptResources || {}
        , resObj = scriptResources[resourceName] || {};
        ...
        return resourceScriptList(resourceName, combine, version, scriptList, minifyEx, copyTo)
        .map(function (path) {
            return generateScript(path, attrs);
        }).join('\r\n');
    };


    var resourceStyleList
        = HtmlHelper.resourceStyleList
        = HtmlHelper.prototype.resourceStyleList
        = function (resourceName, combine, version, styleList, attrs, minifyEx) {
        var appConfig = Mvc.appConfig.appConfig || {}
    ...
    HtmlHelper.prototype.resourceStyle = function (resourceName, combine, version, styleList, attrs, minifyEx) {
        ...
        return resourceStyleList(resourceName, combine, version, styleList, minifyEx)
        .map(function (src) {
            return generateStyle(src, attrs);
        }).join('\r\n');
    }
...
Authorize.prototype.auth = function (next) {
    ...
    ;
    authProvider.isAuth(req, res, next);
}
Authorize.prototype.onUnAuthorize = function (req, res, next, helper) {
    var self = this
    , authProvider = this.provider()
    ;
    authProvider.onUnAuthorize(req, res, next, helper);
}
Authorize.prototype.provider = function () {
    var auth = require('./avril.auth');
    return auth.get(this.controllerConfig().authProvider);
}
Authorize.constructor = Authorize;
//#endregion
...
function Helper(app, req, res, routes, ctrlCfgCache, mvc) {
    ...
    this.authorize = this.auth =new Authorize(app, req, res, routes, ctrlCfgCache, mvc);
