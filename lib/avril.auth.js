var avril = require('./avril')
    ...
    , sysCookies = 'connectId,userStatus,isLogin'.split(',')
    , config = {
        cacheExpire: 60 * 15
    };
    ...
    avril.extend(this, {
        ...
        , isAuth: function () {
            ...
            if (arguments.length == 2) {
                ...
                var connectId = this.connectId(req, res);
                var isLogin = this.cookies.isLogin(req, res);
                if(!isLogin){
                    return next({});
                }
        ...
        , getConnectUser: function (connectId, callback) {
            ...
            avCache.get(key, function (err, user) {
                if (user) {
                    ...
                    avCache.set(key, JSON.stringify(user), function () {
                        callback(user);
                    });
                    avCache.expire(key, config.cacheExpire);
                }
