...
var avril = {
    ...
    , initRootDir: function (dir) {
        var root = this._rootDir = dir || this._rootDir;
        this.executeInitFiles(root);
        return this;
    }

    , executeInitFiles: function(root){
    ...
    * */
    ...
    , require: function (ns) {
        return this.relativeRequire(this._rootDir, ns);
    }

    , areaRequire: function(area, ns) {
        return this.relativeRequire( path.join(this._rootDir, 'areas', area), ns );
    }

    , relativeRequire: function(rootDir, ns) {
        var _path;
        if (ns.indexOf('/') >= 0) {
            _path = path.normalize(path.join(rootDir, ns));
        } else {
            _path = path.normalize(path.join(rootDir, ns.split('.').join('/')));
        }
    ...
    , getConfig: function (name, rootDir) {
        rootDir = rootDir || avril._rootDir;
        var cache = this.configs[name] || (this.configs[name] = {});
        var configPath = path.join(rootDir, 'configs', name + '.json');
        var productionCfgPath = path.join(rootDir, 'configs', name + '.production.json');
        if (!cache[name]) {
            try {
    ...
    , getAreaConfig: function(area, area) {
        return this.getConfig(name, path.join(this._rootDir, 'areas', area));
    }
    ...
    , log: function (text) {
        try {
            ...
            var logFolder = path.dirname(logDir);
            fs.exists(logFolder, function(folderExisted){
                if(folderExisted){
                    writeLogFile();
                }else{
                    fs.ensureDir(logFolder, writeLogFile);
                }
            });
