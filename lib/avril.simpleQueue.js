        ...
        var cbFactory = function(isParal){
            return function ( /* asyncCall ,args..., fn */ ) {
                ...
                (isParal? self.paralFunc: self.func)(function(next) {
					...
					console.log(typeof asyncCall)
					console.log(asyncCall)
					console.log(asyncCall.apply)
        ...
        var eachCbFactory = function(isParal){
            return function(asyncCall , eachArgs, fn) {
				...
				self.func(function(){
					...
					avril.isArray(eachArgs)&& eachArgs.forEach(function(arg) {
						var asyncArgs = standardiseArg(arg);
						asyncArgs.slice(0,0,asyncCall);
						ctx && asyncArgs.slice(0,0,ctx);
						asyncArgs.push(fn);
						console.log('asyncArgs')
						console.log(asyncArgs)
						subQueue[isParal? '$paralAwait' : '$await'].apply(subQueue, asyncArgs);
						//cbFactory(isParal).apply(subQueue, asyncArgs);
					});
					
					console.log('subQueue.length()');
					console.log(subQueue.length());
