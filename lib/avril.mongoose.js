...
Context = {
    ...
    init: function (conn, options) {
        ...
        if(arguments.length == 1) {
            ...
            options = avril.extend(true, defOptions, options);
        }

        !mongoose.connection.readyState && mongoose.connect(conn, options, function (err) {
...
Context.define = function (name, schemadefine, statics, methods) {
    ...
    if (name && schemadefine) {
        ...
        if(mongoose.modelNames().indexOf(name) >= 0){
