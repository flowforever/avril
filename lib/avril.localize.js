...
function _keyValue(culture, str, group) {
    ...
    if (!lanPack.group) { lanPack.group = {}; }
    if (!lanPack.keyValues) { lanPack.keyValues = {}; }
    ...
    , provider: {
        file: {
                ...
                , function (err, file) {
                    if (!err) {
                        ...
                        try {
                            ...
                            callback && callback(localizeObj[language]);
                        } catch (E) {
                            avril.log(E);
                        }
                    }
            ...
            , set: function (language) {
                ...
                var getAdjustedObj = function (obj) {
                    var rtn = {};
                ...
                var savedObj = {
                    group: getAdjustedObj(lan.group)
                    , keyValues: getAdjustedObj(lan.keyValues)
                };

                for (var k in lan.group) {
                    savedObj.group[k] = getAdjustedObj(lan.group[k]);
                }

                var json = JSON.stringify(savedObj, function (key, value) {
                    return value;
                }, 4);
                var self = this;

                    fs.writeFile(self._filePath(language), json, {
                        encoding: 'utf8'
                    });

            }
            , overrided: function (language, newLocalizeObj) {
                localizeObj[language] = localizeObj[language] || {};
                avril.extend(true, localizeObj[language], newLocalizeObj);
                this.set(language);
            }
            , _filePath: function (language) {
                return path.join(this.avril._rootDir, 'languages', language.toLowerCase() + '.txt');
            }
        }
    }
