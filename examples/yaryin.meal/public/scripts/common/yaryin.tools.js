...
; (function ($, yaryin) {
    ...
    (function () {
        yaryin.createlib('yaryin.tools.Localize', function (options) {
            ...
            this.parse = function (force) {
                var localizeItems = $(force ? '[localize]' : '[localize!=true]', context || 'body');

                var languagePack = yaryin.tools.cache.list(language);
                ...
                localizeItems.each(function () {
                    var handle = $(this);
                    if (handle.is('[localize]')) {
                        var item = {
                            text: encodeURI(handle.data(datakey) || handle.html())
                            , group: handle.attr('localize-group')
                        };

                        var cacheItem = languagePack.first(function (lan) {
                            return lan.text == item.text && item.group == lan.group;
                        });

                        if (cacheItem && cacheItem.localize) {
                            handle.html(cacheItem.localize);
                        } else {
                            keys.push(item);
                            handles.push(handle);
                        }
                ...
                if (keys.length > 0) {
                    yaryin.tools.loader.jsonp({
                        url: yaryin.tools.localize.localizeUrl
                        , data: { keys: $.toJSON(keys) }
                        , success: function (localizes) {
                            localizes.each(function (text, index) {
                                text = unescape(text);
                                keys[index].localize = text;
                                handles[index].html(text).attr('localize', 'true');
                                languagePack.push(keys[index]);
                            });
                            languagePack.save();
                        }
                        , error: function () { }
    ...
    //#region node reload
    yaryin.tools.autoReload = function (url) {
        setInterval(function () {
            $.ajax({
                url: url
                , success: function () {
