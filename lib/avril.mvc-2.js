...
function RouteHelper(config) {
    var controllerCaches = {
            constrollers:{}
            , areas: {}
        }
        , seekDir = function(dir, cache){
            fs.readdirSync(dir).forEach(function(ctrlName){
                var controllerDir = path.join( dir, ctrlName );
                fs.statSync(controllerDir).isFile() && (cache[ctrlName] = require(controllerDir));
            });
        };
