...
function copyAndRewriteCssResources(cssFile, sourcePath, destPath) {
    ...
    var paramReg = /\?.*/;
    var getPureFileName = function(url){
        return url.replace(paramReg, '');
    }
    while (exec) {
        (function () {
            var imgPath = exec[1];
            ...
            var imgName = imgNameArr.splice(-5).join('_').replace(/\.\./g,'');
            ...
            imgFile = getPureFileName(imgFile);
            fs.exists(imgFile, function (exist) {
                if (exist) {
                    var destFileName = path.resolve(path.dirname(destPath), resourcesFolder, folderName, imgName);
                    destFileName = getPureFileName(destFileName);
                    fs.mkdirs(path.dirname(destFileName), function (err) {
                        var fs = require('fs');
                        fs.createReadStream(imgFile).pipe(fs.createWriteStream(destFileName));
                    });
                }
            });
        })();
