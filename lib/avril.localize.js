...
var exports = module.exports = {
    ...
    , provider: {
        file: {
            ...
            , get: function (language, localizeObj) {
                fs.readFile(this._filePath(language), 'utf8'
                , function (err, file) {
                    if (!err) {
                        language = language.toLowerCase();
                        try{
                            localizeObj[language] = file ? JSON.parse(file) : {};
                        } catch (E) {
                            avril.log(E);
                        }
            ...
            , set: function (language, localizeObj) {
                ...
                fs.writeFile(this._filePath(language), json, 'utf8');
            }
    ...
    , currentLanguage: function (req, res) {
        var auth = avril.auth.instances.default;
